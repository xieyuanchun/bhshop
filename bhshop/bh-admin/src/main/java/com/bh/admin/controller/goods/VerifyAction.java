...
import com.bh.admin.pojo.user.VilidatePhone;
...
public class VerifyAction {
	...
	public BhResult checkPhoneCode(@RequestBody Map<String, String> map, HttpServletRequest request,
		...
		//获得session的对象
		Object o = request.getSession().getAttribute(SmsContent);
		if (o != null) {
	...
	public void getViladate (HttpServletResponse response,HttpServletRequest request) throws Exception{
		...
		//HttpServletResponse response = (HttpServletResponse) RequestContextHolder.currentRequestAttributes(); //ServletActionContext.getResponse();
		/** 设置响应的内容类型 */
		...
		System.out.println("图形验证码的文字------->" + code);
		sesson.setAttribute(VERIFY_CODE, code);
		String co = (String) request.getSession().getAttribute(VERIFY_CODE);
		System.out.println("在verifyAction类中的request.getSession().getAttribute(VERIFY_CODE)------->" + co);
		/** 消毁画笔 */
		g.dispose();
		/** 输出 */
		String targetFileName = StringUtil.genUuId() + ".jpg";
		StringBuffer curProPath = request.getRequestURL();
		
		String imgPath = curProPath+targetFileName;
		...
		bhResult = new BhResult(200, "请求成功", image);
		//return bhResult;
	}
	/**
	 * 画笔位置倾斜方法
