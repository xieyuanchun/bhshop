...
import com.bh.admin.service.GoodsCommentService;
import com.bh.config.Contants;
import com.bh.admin.mapper.goods.GoodsCommentMapper;
...
public class GoodsCommentImpl implements GoodsCommentService{
		...
		for(GoodsComment goodsComment : list){
			...
			if(goodsComment.getNotname()==0){
				...
				if(member.getUsername()!=null){
					goodsComment.setmName(URLDecoder.decode(member.getUsername(),"utf-8"));
				}else{
					goodsComment.setmName("匿名");
				}
				...
				if(user!=null){
					goodsComment.setmName(URLDecoder.decode(user.getName(),"utf-8"));
	...
	public PageBean<GoodsComment> pageByShopId(GoodsComment entity, int shopId)
		...
		if(list.size()>0){
			for(GoodsComment comment : list){
				try {
					if (org.apache.commons.lang3.StringUtils.isNotBlank(comment.getDescription())) {
						comment.setDescription(URLDecoder.decode(comment.getDescription(),"utf-8"));
					}
				} catch (Exception e) {
				...
				if(member!=null){
					if(comment.getNotname()==0){
						...
						comment.setmName(URLDecoder.decode(member.getUsername(),"utf-8"));
						comment.setmHead(member.getHeadimgurl());
					}
				}else{
					PromoteUser user = promoteMapper.selectByPrimaryKey(comment.getmId());
					if(user!=null){
						comment.setmName(URLDecoder.decode(user.getName(),"utf-8"));
	...
	public int insert(GoodsComment entity) throws Exception {
		...
		goods.setComments(goods.getComments()+1);//评论数+1
		goodsMapper.updateByPrimaryKeySelective(goods);//xieyc
