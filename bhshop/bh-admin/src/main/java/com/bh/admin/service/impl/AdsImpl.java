...
import com.bh.admin.mapper.goods.AdsMapper;
import com.bh.admin.mapper.goods.AdsRelationMapper;
import com.bh.admin.mapper.goods.MenuMapper;
...
public class AdsImpl implements AdsService{
	 ...
	 */
	@Override
	public int editAds(Ads entity) throws Exception {
		int row = 0;
		Ads ads=mapper.selectByPrimaryKey(entity.getId());
		entity.setCreatetime(ads.getCreatetime());
		entity.setStatus(ads.getStatus());
		if(entity.getSortnum()!=null && ads.getSortnum()!=entity.getSortnum()){
			List<Ads> adsList = mapper.selectListBySortNum(entity.getSortnum());
			if(adsList.size()>0){
				List<Ads> list = mapper.selectAll(); //获取所有排序号，从小到大排序
				if(list.size()>0){
					for(int i=0; i<list.size()-1; i++){
			...
			entity.setSortnum(entity.getSortnum());
		}
		if(entity.getIsMain()!=null){
			if(ads.getIsMain()==entity.getIsMain()){ //正常情况
				entity.setIsMain(entity.getIsMain());
			}else{ 
				if(ads.getIsMain()==0){ //pc改移动
					entity.setStatus(0); //0正常
					entity.setIsMain(entity.getIsMain());
				}else{ //移动改pc
					entity.setStatus(0); //0正常
					entity.setIsMain(entity.getIsMain());
		...
		row = mapper.updateByPrimaryKey(entity);
		return row;
	}
