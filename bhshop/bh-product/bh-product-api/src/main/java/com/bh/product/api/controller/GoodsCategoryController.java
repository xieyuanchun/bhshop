...
public class GoodsCategoryController {
	...
	public BhResult updateCategory(@RequestBody Map<String, String> map){
		...
		try {
			...
			String image = map.get("image");
			String flag = map.get("flag");
			int row = service.updateCategory(id, name, sortnum, image, flag);
			if (row == 1) {
				result = new BhResult(200, "添加成功", null);
			} else if(row ==1000){
				result = BhResult.build(400, "名称已存在");
	...
	public BhResult selectTopSixGoodsBycatId() {
		BhResult result = null;
		try {
			List<MyGoodsCategoryPojo> goodsCategory = service.selectTopSixGoodsBycatId();
			if(goodsCategory!=null){
				result = new BhResult(200, "成功", goodsCategory);
			}else{
				result = BhResult.build(400, "暂无数据！");
	 ...
	 * @return
	 */
	@RequestMapping("/selectLinkedList1")
	@ResponseBody
	public BhResult selectLinkedList1() {
		BhResult result = null;
		try {
...
//			Map<String, Object> list = service.getCateListAndGoods(Contants.PAGE_SIZE, currentPage, catId);
			//2018.5.30 zlk
			Map<String, Object> list = service.getCateListAndGood(Contants.PAGE_SIZE, currentPage, catId);
			//end
			result = new BhResult(BhResultEnum.SUCCESS, list);
		} catch (Exception e) {
			result = new BhResult(BhResultEnum.VISIT_FAIL, null);
	...
	public BhResult getGoodsByCatId(@RequestBody Map<String, String> map) {
		...
		try {
			String currentPage = map.get("currentPage");
			String catId = map.get("catId");
			PageBean<Goods> page = service.getGoodsByCatId(currentPage,catId); 
			r = new BhResult();
			r.setStatus(BhResultEnum.SUCCESS.getStatus());
			r.setMsg(BhResultEnum.SUCCESS.getMsg());
			r.setData(page);
	...
	/**
	 * @Description: 获取所有某个catone下的所有二级和三级
	 * @author xieyc
	 * @date 2018年8月10日 下午5:01:53 
	 * @param   
	 * @return  
	 */
	@RequestMapping("/selectLinkedByCatIdOne")
	@ResponseBody
	public BhResult selectLinkedByCatIdOne(@RequestBody Map<String, String> map) {
		BhResult result = null;
		try {
			List<GoodsCategory> msg = service.selectLinkedByCatIdOne(map.get("catIdOne"));
			if(msg!=null){
				result = new BhResult(200, "成功", msg);
			}else{
				result = BhResult.build(400, "暂无数据！");
	...
	/**
	 * @Description: 获取所有一级分类列表(树形结构)
	 * @author xieyc
	 * @date 2018年8月10日 下午9:20:39 
	 */
