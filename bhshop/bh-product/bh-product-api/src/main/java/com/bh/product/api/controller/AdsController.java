...
public class AdsController {
	...
	public BhResult addAds(@RequestBody Map<String, String> map){
		...
		try {
			...
			String sLink = map.get("sLink"); //广告链接
	...
	@ResponseBody
	public BhResult editAds(@RequestBody Map<String, String> map){
		BhResult result = null;
		try {
			...
			String isMain = map.get("isMain"); //是否是首页广告0不是，1是
			String isPc = map.get("isPc"); //0-pc端，1-移动端
			int row = service.editAds(id, name, image, content, link,sLink, sortnum, isMain, isPc);
			if(row == 0){
