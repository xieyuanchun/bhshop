...
import com.bh.product.api.service.SeedScoreRuleService;
	...
	public BhResult updateScore(SeedScoreRule rule) throws Exception {
		...
		if (rule.getStatus() == null) {
			...
			if (rule.getId().equals(1)) {
				...
				// 积分动作 1，签到 2,浇水 3,拼团 4单买 5,分享,6注册积分，7购物积分
				ScoreRuleExt ext = rule.getList().get(0);
				...
				if (ext.getId() != null) {
					...
					ext.setSrId(rule.getId());
					ext.setIsDel(0);
					scoreRuleExtMapper.updateScoreRuleExtByValue(ext);
				} else {
					// 不存在，需要insert
					ext.setSrId(rule.getId());
					ext.setIsDel(0);
