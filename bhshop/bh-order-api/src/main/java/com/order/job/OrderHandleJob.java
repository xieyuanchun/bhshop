 ...
 */
@JobHander(value="orderHandleJob")
@Component
public class OrderHandleJob extends IJobHandler {
	...
	public synchronized ReturnT<String> execute(String... params) throws Exception {
		...
		noticeReq.setMsgType(1);
		List<MemberNotice> list = memberNoticeMapper.selectMemberNoticeList(noticeReq);
		for (MemberNotice mn:list) {
			try {
				String url = baseUrl+"?mId="+mn.getmId()+"&message="+mn.getMessage()+"&goodsId="+mn.getGoodsId();
